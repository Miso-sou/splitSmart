...
export default function WakeupGate({ children }) {
  ...
  // Show waking modal only if backend did not respond on the immediate first attempt (e.g. Render cold start)
  const isSleeping = status === 'waking' && attempt > 1;

  return (
    <>
      {children}
      {(isSleeping || status === 'error') && (
        <div className="fixed inset-0 flex items-center justify-center bg-[#0f1010]/90 backdrop-blur-md z-50 transition-opacity duration-300">
